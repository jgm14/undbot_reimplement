...
def load_label(filename):
    ...
    csv_file = open(filename + "/label.csv", "w+", newline='')
...
def process_json(filename):
    csv_file = open(filename + "/" + filename + ".csv", "w+", newline='')
    writer = csv.writer(csv_file)
    header = ["userid", "screenname", "follower", "friend"]
    writer.writerow(header)
    ...
    for data in tweets:
        ...
        while flag != 3:
            if data[i].isdigit():
                ...
                infs[x] += int(text[-1])
                flag += 1
    with open(filename + '/' + filename + '_f.csv', "w+", newline='') as out_file:
        writer = csv.writer(out_file)
        header = ["userid", "label", "infs", "ff", "type_o", "type_r", "type_c"]
        writer.writerow(header)
        for i in range(num):
            if sum(types[i]) != 0:
...
def multirank2(multi_graph, stabalize_threshold):
    # multi_graph: [r, m, m]
    # print('----------------------------------------------')
    print('is calculating multirank...')
    m = len(multi_graph[0])
