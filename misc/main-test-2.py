...
def test(bot_filename, human_filename, dataset_path=''):
    ...
    graph_ff = csr_matrix(mask_ff.astype(float))
    graph_ff -= csr_matrix(np.diag(np.diag(graph_ff.toarray())))
    ...
    graph_types = csr_matrix(mask_types.astype(float))
    graph_types -= csr_matrix(np.diag(np.diag(graph_types.toarray())))
    ...
    graph_infs = csr_matrix(mask_infs.astype(float))
    graph_infs -= csr_matrix(np.diag(np.diag(graph_infs.toarray())))

    max_ff[~mask_ff] = 1
    max_infs[~mask_infs] = 1

    adj_matrix = (graph_ff.multiply(1 - (diff_ff / max_ff)) +
                  graph_types.multiply(1 - diff_types) +
                  graph_infs.multiply(1 - (diff_infs / max_infs))).toarray()
